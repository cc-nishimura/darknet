...
if os.name == "nt":
    ...
    cwd = os.path.dirname(__file__)
    if os.path.exists(os.path.join(cwd, "..", "darknet.so")):
        lib = CDLL(os.path.join(cwd, "..", "darknet.so"), RTLD_GLOBAL)
        repo = "alexeyab"
    else:
...
def detect(net, meta, image, thresh=.5, hier_thresh=.5, nms=.45, debug=False):
    ...
    # import scipy.misc
    # custom_image = scipy.misc.imread(image)  # you should comment line below: free_image(im)
    ...
    if repo == "alexeyab":
        ...
        #     net, custom_image_bgr.shape[1], custom_image_bgr.shape[0], thresh, hier_thresh, None, 0, pnum, 0)  # OpenCV
        dets = get_network_boxes(
...
def performDetect(imagePath="data/dog.jpg", thresh=0.25, configPath="./cfg/yolov3.cfg", weightPath="yolov3.weights", metaPath="./data/coco.data", showImage=True, makeImageOnly=False, initOnly=False):
    ...
    if netMain is None:
        if repo == "alexeyab":
            ...
            netMain = load_net(configPath.encode(
                "ascii"), weightPath.encode("ascii"), 0)  # batch size must set to 1 in .cfg file
    if metaMain is None:
